...
const { server: { id, role_ids, mod_log_id } } = require("/home/runner/Discord-Bot-List/config.json")
...
route.post("/:id", auth, async function (req, res) {
    ...
    modLog.send(
        new MessageEmbed()
            ...
            .addField(`Bot`, `</home/runner/Discord-Bot-List/${bot.botid}>`, true)
            .addField(`Owner(s)`, owners.map(x => x ? `</home/runner/Discord-Bot-List/${x}>` : ""), true)
            .addField("Mod", req.user.username, true)
            .setThumbnail(botUser.displayAvatarURL({format: "png", size: 256}))
            .setTimestamp()
            .setColor(0x26ff00)
        );
    modLog.send(owners.map(x => x ? `</home/runner/Discord-Bot-List/${x}>` : "")).then(m => { m.delete() });
